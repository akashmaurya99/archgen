...
//
//   npx archgen init            Install skill into THIS project + write
//                               AGENTS.md / CLAUDE.md context pointers
//   npx archgen install         Install skill into global harness dirs
//                               (Claude Code, OpenCode, Cursor, agentskills)
//   npx archgen uninstall       Remove globally-installed copies (manifest-based)
//
...
Usage:
  npx archgen init [dir]        Prepare a project: copy the skill into
  ...
  npx archgen install           Install into global harness skill dirs
    [--copy]                      real copies instead of symlinks
    [--uninstall]                 remove manifest-recorded entries
  npx archgen uninstall         Alias for install --uninstall
  npx archgen --help            This help
...
try {
  switch (cmd) {
    ...
    case 'install': {
      ...
      console.log('\nPer-project setup (recommended): npx archgen init');
